...
//-----------Begin Statusabfrage (neue SollPositionen für Motoren)------------------- 
        ...
        if ((G_Change == 1)&&(DoneFlag != 1)){      //Wenn Gewichtsänderung dann Motor-Werte aktualisieren           
          if      (WaageBeladen >= 3)  soundAmb(5);
          else if (WaageBeladen >= 2)  soundAmb(6);
          else if (WaageBeladen >= 1)  soundAmb(7);
    
          if      (WaageBeladen == 0)  soundAmb(8);
          ...
          for (int i=1; i<=4;i++)  Durchlauf[i] = 0;                                //Für Nachschwingdurchgänge beim Bewegen      
        }
 ...
 //-------------- Sonderfall Rätsel gelöst  ----------------------------------------------
        ...
        if (StatusNeu == 13)  Waagezeit_inaktiv = millis();  //Falls StatusÄnderung Reset Waagezeit_inaktiv (für Restart nach bestimmter Inaktivzeit)
...
//------------------Begin Motorfahrt / Fahrtkontrolle-------------------------------------
        ...
        (M_Done[4] == true)){
          ...
          textWarter++;
          int textTrigger = textWarter % 3;  // loop from 0 to 2
        ...
        (M_Done[3] == true) && 
        (M_Done[4] == true))
          soundAmb(8);  //Music  StopChannelsWaagenSound; //Kanal 1 - 3 ausschalten
         
        if(Gewicht[4] > G_SchwellwertMin + 2){ // Falsche Zutate Handling
          if     (Gewicht[4] > G4_Schwelle3)    soundStatus = 13;
          else if(Gewicht[4] > G4_Schwelle2)    soundStatus = 12;
          else                                  soundStatus = 11;
        }
        else{ // Wenn keine falsche Zutat.
          if      (G_Zustand == 3)    soundStatus = 8;
          else if (G_Zustand == 2)    soundStatus = 7;
          else if (G_Zustand == 1)    soundStatus = 6;
          else{
            if      (StatusNeu == 13) soundStatus = 1;  //stand by Status
            else if (StatusNeu == 14) soundStatus = 0;  // In other place applyied. //Loesung
            else                      soundStatus = 2;  // Andere Status 
          } 
        }
    
        if(soundStatus < soundStatus_alt){
          ...
          else  soundStatus_do = soundStatus_alt;
        }
        else{    
